...
class HLG(object):
  ...
  def __init__(self,distance_type, f=3, m=16, ef=200, m0=None, heuristic=True, vectorized=False):
    if distance_type == "l2":
      # l2 distance
  ...
  def build_index(self, vectors, f, ef=None):
    distance = self.distance
    m0 = self._m0
    ...
    graphs = self._graphs
    t = time.time()
    layered_dict,num_layers = self.layering(vectors,f)
    ...
    for j in layer_val_map.keys():
      for k in layer_val_map.keys():
        if j<k:
          ...
          next = layer_val_map[k]
          for a in current:
            ngbrs = self.search(a,m0)
            for nb in ngbrs:
              if nb[0] in next:
  ...
  def layering(self,vectors,f):

    m = self._m
    num_layers = int(log2(m))
    lower_bound, upper_bound, distances = filter_outlier.reject_outliers(vectors,f)
  ...
  def _search_next(self, g, q, k=None, ef=None):
    """Find the k points closest to q."""
